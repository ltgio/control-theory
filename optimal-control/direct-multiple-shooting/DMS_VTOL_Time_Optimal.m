...
import casadi.*
%% Parameters =============================================================
mass  = 3*10^3;           % massa veivolo [kg]
J     = 3*10^3;           % inerzia [kg*m^2]
...
% Plot the solution
y_opt       = w_opt(1:nx+nu:end-1);
z_opt       = w_opt(2:nx+nu:end-1);
theta_opt   = w_opt(3:nx+nu:end-1);
dy_opt      = w_opt(4:nx+nu:end-1);
dz_opt      = w_opt(5:nx+nu:end-1);
dtheta_opt  = w_opt(6:nx+nu:end-1);

T = w_opt(end);

T_opt       = w_opt(7:nx+nu:end-1);
F_opt       = w_opt(8:nx+nu:end-1);
