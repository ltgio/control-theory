...
#if ACADO_NYN
	getArray(1, src, 0, "yN", acadoVariables.yN, ACADO_NYN, 1);
#endif /* ACADO_NYN */
...
#if (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_QPOASES)
...
#endif /* (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_QPOASES) */
	...
	switch ( ctrl )
	{
		case 0:
			...
			else if (strategy == 1 || strategy == 2)
			{
				...
				shiftStates(strategy, xEnd, uEnd);
				shiftControls(uEnd);
			}
			
			preparationStep();
			
			status = (double)feedbackStep();
			
			kktValue = getKKT();
			objValue = getObjective();

#if ACADO_QP_SOLVER == ACADO_QPOASES
			nIterations = (double)getNWSR();
