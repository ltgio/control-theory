...
xeq  = [sqrt(km/(m*g0))*(ueq/r), 0 ,ueq/r];    % equilibrium state
X0   = [0 0 0]+xeq;                            % initial state
ueq2 = 10;                                     % equilibrium input [volts]
xeq2 = [sqrt(km/(m*g0))*(ueq2/r), 0 ,ueq2/r];  % equilibrium state
% Xref = [0 0 0]+xeq2                          % reference point
...
input.y = [sine_fun([0:N-1]*Ts).' zeros(N,1)]; % reference trajectory for the stage cost
input.yN = sine_fun(N*Ts);                     % reference trajectory for the terminal cost                 
input.W = diag([1 1e-4]);                      % shaping matrix values blqdiag
input.WN = diag([1]);                          %                                                                                                             
...
while time(end) < Tf
    ...
    input.x = output.x;
    input.u = output.u;
    input.y = [input.y(2:end,:); [input.yN.' 0]];
    input.yN = sine_fun(N*Ts+nextTime);
...
figure(1);
subplot(2,1,1);plot(time,state_sim,'LineWidth',2);hold on;
plot(time,x2max*ones(length(time)),'r--');grid on;legend('x1','x2','x3','speed constraint');
subplot(2,1,2);stairs(time(1:end-1),controls_MPC);hold on
