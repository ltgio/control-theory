...
import casadi.*
global T N nx nu                    % make global variable for Callback
%% Parameters =============================================================
...
p    = MX.sym('p',0,1);             % no parametric variable
...
Call = MyCallback('f', nw, ng, np);

% Plot the solution
y_opt       = w0(1:nx+nu:end);
z_opt       = w0(2:nx+nu:end);
theta_opt   = w0(3:nx+nu:end);
dy_opt      = w0(4:nx+nu:end);
dz_opt      = w0(5:nx+nu:end);
dtheta_opt  = w0(6:nx+nu:end);

T_opt       = w0(7:nx+nu:end);
F_opt       = w0(8:nx+nu:end);
...
prob = struct('x', vertcat(w{:}), 'p', p, 'f', J, 'g', vertcat(g{:}));
% option IPOPT
